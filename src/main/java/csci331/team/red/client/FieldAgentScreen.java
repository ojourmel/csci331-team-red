...
public class FieldAgentScreen implements Screen {
	...
	// Used for controlling stuff moving around. Going to be moved into the
	// game.
	...
	public FieldAgentScreen(ClientEngine parent) {
		...
		backgroundImage = parentEngine.gameTextureManager
				.get(parentEngine.Textures.get("level1fieldbg"));

		// Loads other textures
		clipBoard = parentEngine.gameTextureManager.get(parentEngine.Textures
				.get("clipboard"));

		characters = new HashMap<String, Pixmap>();

		characters.put("thug", parentEngine.gamePixmapManager
				.get(parentEngine.Pixmaps.get("level1thug")));
		characters.put("female", parentEngine.gamePixmapManager
				.get(parentEngine.Pixmaps.get("level1female")));
		characters.put("male", parentEngine.gamePixmapManager
				.get(parentEngine.Pixmaps.get("level1male")));
		...
		passiveState.setPosition(
				Gdx.graphics.getWidth() - passiveState.getWidth(),
				Gdx.graphics.getHeight() - passiveState.getHeight());
		states.add(passiveState);
		uiStage.addActor(passiveState);

		aggressiveState = new TextButton("Aggressive",
				parentEngine.radioButtonStyle);
		aggressiveState.setPosition(
				Gdx.graphics.getWidth() - aggressiveState.getWidth(),
				passiveState.getY() - aggressiveState.getHeight());
		states.add(aggressiveState);
		uiStage.addActor(aggressiveState);

		detainButton = new TextButton("Detain!", parentEngine.buttonStyle);
		detainButton.setPosition(
				Gdx.graphics.getWidth() - detainButton.getWidth(), 0);
		detainButton.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				String[][] strarr = {
						{
								"Oh dear, sweetie... it seems you've forgotten your cuffs",
								"Ominious Voice" },
				...
				fieldDialogCallbacks.callbacks[] callarr = { null, null, null,
						null };
		...
		allowButton.setPosition(
				Gdx.graphics.getWidth() - allowButton.getWidth(),
				detainButton.getHeight());
		allowButton.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				...
				fieldDialogCallbacks.callbacks[] callarr = { null, null,
						fieldDialogCallbacks.callbacks.approachSecondPerson,
				...
				Tween.to(currentPerson, ActorTweener.POSITION_XY, 1.0f)
						.target(-500, -500).ease(Quad.IN).start(tweenManager);
				Tween.to(ticket, ActorTweener.POSITION_XY, 1.0f)
						.target(-500, ticket.getY()).ease(Quad.IN)
						.start(tweenManager);
		...
		fieldDialogCallbacks.callbacks[] callarr = { null, null,
				fieldDialogCallbacks.callbacks.approachFirstPerson, null, null,
				null, null, fieldDialogCallbacks.callbacks.giveID, null, null,
				null, null, null, null };
		Dialog[] d = Dialog.returnDialogArray(strarr, callarr);
		displayDialogue(d);
	...
	private void displayNewPerson(final int currentdude) // This is just bad and
															// hardcoded. The
															// server will
															// provide a
															// 'person' in the
															// future.
	{
		...
		if (currentdude == 1) {
			currentPerson = new TransparentActor(characters.get("female"),
					tweenManager);

			charactersStage.addActor(currentPerson);
			Tween.to(currentPerson, ActorTweener.POSITION_XY, 1.0f)
					.target(300, 300).ease(Quad.IN).start(tweenManager);

			// Tween.to(thug,ActorTweener.SCALE_XY, 3.0f).target(2 ,
			// 2).ease(Quad.IN).start(tweenManager);

			// Tween.to(thug,ActorTweener.ZOOM,
			// 3.0f).target(thug.getX()-thug.getWidth()/2
			// ,thug.getY()-thug.getHeight()/2 , 2 ,
			// 2).ease(Quad.IN).delay(2).start(tweenManager);

			Tween.to(currentPerson, ActorTweener.ZOOM, 3.0f)
					.target(300 - currentPerson.getWidth() / 2,
							(300 - currentPerson.getHeight() / 2) - 200, 2, 2)
					.ease(Quad.IN).delay(2).start(tweenManager);
		}

		else if (currentdude == 2) {

			currentPerson = new TransparentActor(characters.get("thug"),
					tweenManager);

			ticket = new TransparentActor(
					parentEngine.gamePixmapManager.get(parentEngine.Pixmaps
							.get("goldenTicket")), tweenManager);
			ticket.setDragable();
			ticket.scale(-1);
			ticket.setPosition(300, 400);
			Tween.to(ticket, ActorTweener.ZOOM, 1.0f).target(200, 100, 1, 1)
					.ease(Quad.IN).delay(6).start(tweenManager);
	...
	// This is static because... I wanted to enum and couldn't figure out how to
	// enum without it being static.
	// =<
	...
	public void produceDocument(int documentNumber) {
		...
		ticket.setPosition(300, 400);
		Tween.to(ticket, ActorTweener.ZOOM, 1.0f).target(200, 100, 1, 1)
				.ease(Quad.IN).start(tweenManager);

		papersStage.addActor(ticket);

	}

	// TODO: Move dialogue functions into somewhere shared between the two agent
	// stages. Maybe.
	...
	public void displayDialogue(Dialog[] dialogueArray) {
		...
		if (dialogueArray.length > 1) {
			iteratorOld = new DialogueWindow(
					dialogueArray[dialogueArray.length - 1].getDialogue(),
					dialogueArray[dialogueArray.length - 1].getSpeaker(),
					parentEngine.dialogueStyle, dialogueStage, true, 20, false,
					null,
					dialogueArray[dialogueArray.length - 1].getCallbackCode());

			for (int i = dialogueArray.length - 2; i > 0; i--) {
				DialogueWindow iteratorNew = new DialogueWindow(
						dialogueArray[i].getDialogue(),
						dialogueArray[i].getSpeaker(),
						parentEngine.dialogueStyle, dialogueStage, true, 20,
						false, iteratorOld, dialogueArray[i].getCallbackCode());
				iteratorOld = iteratorNew;
			}
		}
		new DialogueWindow(dialogueArray[0].getDialogue(),
				dialogueArray[0].getSpeaker(), parentEngine.dialogueStyle,
				dialogueStage, true, 20, true, iteratorOld,
				dialogueArray[0].getCallbackCode());
