...
public class TransparentActor extends Actor {
	...
	public void setDragable() {
		...
		this.addListener((new DragListener() {
			public void touchDragged(InputEvent event, float x, float y,
				...
				event.getTarget().setPosition(event.getTarget().getX() + dx,
						event.getTarget().getY() + dy);
	...
	public void draw(SpriteBatch batch, float parentAlpha) {
		...
		batch.draw(texture, getX(), getY(), getOriginX(), getOriginY(),
				texture.getWidth(), texture.getHeight(), getScaleX(),
				getScaleY(), getRotation(), 0, 0, texture.getWidth(),
