...
public class HostingScreen implements Screen {
	...
	public HostingScreen(ClientEngine parent) {
		...
		// Creates our buttons
		TextButton HostingLabel = new TextButton("Stay a while... and listen.",
		...
		TextButton HostnameLabel = new TextButton("Hostname: "
				+ localhost.getHostName(), parentEngine.buttonStyle);
		hostingStage.addActor(HostnameLabel);

		TextButton BackButton = new TextButton("Stop Hosting",
				parentEngine.buttonStyle);
		...
		HostingLabel
				.setPosition(
						Gdx.graphics.getWidth() / 2 - HostingLabel.getWidth()
								/ 2,
						(Gdx.graphics.getHeight()
								/ (hostingStage.getActors().size + 1) * 3));
		HostnameLabel
				.setPosition(
						Gdx.graphics.getWidth() / 2 - HostnameLabel.getWidth()
								/ 2,
						(Gdx.graphics.getHeight()
								/ (hostingStage.getActors().size + 1) * 2));
		BackButton
				.setPosition(
						Gdx.graphics.getWidth() / 2 - BackButton.getWidth() / 2,
						(Gdx.graphics.getHeight()
								/ (hostingStage.getActors().size + 1) * 1));
