...
public class DatabaseAgentScreen implements Screen {
	...
	public DatabaseAgentScreen(ClientEngine parent) {
		...
		backgroundImage = parentEngine.gameTextureManager
				.get(parentEngine.Textures.get("level1databasebg"));
		...
		computerTextBox.setTextFieldListener(new TextFieldListener() {
			public void keyTyped(TextField textField, char key) {
				...
				if (key == '\n' || key == '\r') {
					...
					String computerResponse = computerSearch(textField
							.getText().trim());
					...
					computerTextLabelContainingTable.add(newCommandLabel)
							.left();

					computerTextLabelContainingTable.row();

					computerTextLabelContainingTable.add(newResponseLabel)
							.left();
		...
		String[][] strarr = {
						...
						"Ominious Voice" },
				{
						...
						"Ominious Voice" },
				{
						"Your parnter will also be reporting suspicious data to you to look up.  For example...",
						"Ominious Voice" },
		...
		DatabaseDialogCallbacks.callbacks[] callarr = { null,
				DatabaseDialogCallbacks.callbacks.startAlerts, null,
				DatabaseDialogCallbacks.callbacks.MaryTestAlert, null,
				DatabaseDialogCallbacks.callbacks.startAlerts, null, null,
				null, null, null, null };
		...
		String[] test = { "Person of interest spotted",
				"Person of interest spotted", "Person of interest spotted",
				"Person of interest spotted", "Person of interest spotted",
				"Hostile individual wearing 'thug life' tshirt",
				"VUI Alerts System Reset",
				"Potential threat from person here for 'two weeks'"
	...
	public void addAlert(String alertText) {
		...
		Label newCommandLabel = new Label("VUI ALERT: " + alertText,
				parentEngine.rawTextStyle);
		newCommandLabel.setWrap(true);
		float alertScreenLength = alertTextScrollingTable.getWidth();

		alertTextLabelContainingTable.row();
		alertTextLabelContainingTable.add(newCommandLabel).left()
				.minWidth(alertScreenLength - 20).fill();
